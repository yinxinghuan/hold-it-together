...
Hold It Together 是一个 `casual` 类型的移动端小游戏：AlterU After Dark「撑住别崩」—— 内耗生存模拟。Reigns 式左右滑卡: 老板深夜消息/老妈催婚/房东涨租/同事甩锅……每张卡两个选项, 选哪个都得拿一样换另一样, 心力与钱包此消彼长。卡中央那张脸就是你, 心力越掉脸越垮 (在线→疲惫→内耗→一点就炸→已崩), 任一归零 game over, 比谁撑得久。即点即懂。

## 2. Visual Design

- 整体布局：页面占用 100vw x 100vh，主体验居中，HUD 与操作区覆盖在游戏层上方，移动端以单手操作为优先。
- 背景与配色：主要颜色使用 #FFD700、#5ad1c7、rgba(255,255,255,0)、#15151c、#fff、#f0b341、rgba(255, 255, 255, 0.1)、rgba(255, 255, 255, 0.06)；高亮元素用于可点击目标、得分、结果或稀有状态。
